...
async def update_user(
    ...
    if user.email:
        db_user_email: models.User = await crud.get_user_by_email(db=db, email=user.email)
        if db_user_email:
            raise HTTPException(
                status_code=400,
    ...
    if db_user.email and user.email:
        db_token: models.Confirmation = await crud.get_confirmation(db=db, email=db_user.email, option="MODIFY", time_delta=300)
        db_token1: models.Confirmation = await crud.get_confirmation(db=db, email=user.email, option="BIND", time_delta=300)
    ...
    if db_user.email is None and user.email:
        db_token: models.Confirmation = await crud.get_confirmation(db=db, email=user.email, option="BIND", time_delta=300)
        if db_token is None or db_token.token != email_token:
            raise email_token_exception
    
    if user.password:
        db_token: models.Confirmation = await crud.get_confirmation(db=db, email=user.email, option="MODIFY", time_delta=300)
        if db_token is None or db_token.token != email_token:
            raise email_token_exception
