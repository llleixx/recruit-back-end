...
class TestUsers:
    ...
    def test_send_email(self):
        response: httpx.Response = self.client.post(
            "/sendEmail",
            json={"option": "BIND", "email": "llleigoing@outlook.com"}
        )
        assert response.status_code == 200
        assert response.json() == {
            "detail": "SUCCESS"
...
class TestProblems:
    ...
    def test_post_problem(self):
        response: httpx.Response = self.client.post(
            "/problems/",
            json={"name": "que1", "description": "que1", "answer": "123", "score_initial": 1000}
        )
        assert response.status_code == 200
        assert response.json() == {
            "name": "que1",
